...
export const contextualGlobalSearch = async (query, context = 'all') => {
  if (!query || query.trim().length === 0) {
    return { contacts: [], companies: [], deals: [], tasks: [], leads: [] };
  }
  ...
  try {
    ...
    const [contacts, companies, deals, tasks, leads] = await Promise.all([
...
const companyLimits = getResultLimits('companies');
    ...
    const leadLimits = getResultLimits('leads');
    ...
    return {
      ...
      leads: leads.slice(0, leadLimits.primary)
    };
  } catch (error) {
